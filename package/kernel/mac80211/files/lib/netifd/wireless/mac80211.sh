...
drv_mac80211_init_device_config() {
	...
	config_add_int beacon_int chanbw frag rts wifi_retry
...
drv_mac80211_setup() {
	...
	json_get_vars \
		...
		rxantenna txantenna \
		frag rts beacon_int:100 htmode wifi_retry
	...
	[ -n "$wifi_retry" ] && iw phy $phy set retry long $wifi_retry short $wifi_retry
	...
	[ -n "$hostapd_ctrl" ] && {
		...
		if [ "$no_reload" != "0" ]; then
			...
			/usr/sbin/hostapd -P /var/run/wifi-$phy.pid -B -t -f /tmp/run/hostapd.log "$hostapd_conf_file"
			ret="$?"
